...
export default function CartSticker() {
  ...
  const checkout = () => {
    // vai al checkout dedicato
    window.location.href = "/user/checkout";
  };

  // Se vuoi che lo sticker compaia anche su pagine senza route /cart,
  // puoi spostare questo componente a livello di layout/app.
  return (
              ...
              {items.map((item, idx) => (
                <li key={idx} className="cart-sticker__item">
                  ...
                  <div className="item__line">
                    <span>Base</span>
                    <strong>{eur.format(item.basePrice)}</strong>
                  </div>

                  {item.options.length > 0 && (
                    <details className="item__options" aria-label="Opzioni">
